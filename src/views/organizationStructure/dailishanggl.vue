      ...
      <div class="m-listTitleFoot" v-if="searchList.length!=0">
        <el-row>
          ...
          <el-col v-if="false" :span="12">
            <div class="grid-content bg-purple-light fr">操作:<el-button size="mini" @click="doFunction('offLine')">强制离线</el-button>
              <el-button size="mini" @click="doFunction('addBlack')">加入黑名单</el-button>
              <el-button size="mini" @click="doFunction('cancelBlack')">解除黑名单</el-button>
              <el-button size="mini" @click="doFunction('delete')">删除</el-button>
            </div>
          </el-col>
        </el-row>
      </div>
        ...
        <el-form :model="addForm" :rules="rules" ref="addForm" label-width="80px">
          <el-form-item label="归属渠道" prop="addDepId">
            <el-select style="width:400px;" size="mini" v-model="addForm.addDepId" placeholder="请选择归属渠道">
              <el-option v-for="item in depart" :key="item.value" :label="item.label" :value="`${item.dealerIdName}`+`【${item.dealerId}】`">
              </el-option>
            </el-select>
          </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        addDepPhone: [{
          required: true,
          message: "部门联系人电话为必填内容",
          trigger: "blur"
        }],
        detailAddress: [{
          required: true,
          message: "收货人详细地址为必填内容",
          trigger: "blur"
        }],
      },
      off: {
